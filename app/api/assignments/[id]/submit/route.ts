  ...
  try {
    ...
    if (!activeSubscription) {
      return NextResponse.json(
        {
          success: false,
          message: "يجب أن يكون لديك اشتراك فعال لاستخدام الواجبات.",
        },
        { status: 403 }
      );
    }
    ...
    if (!assignment.isPublished) {
      return NextResponse.json(
        {
          success: false,
          message: "هذا الواجب غير متاح حاليًا.",
        },
        { status: 403 }
      );
    }
    ...
    if (
      !assignment.lesson ||
      !assignment.lesson.isPublished ||
      !assignment.lesson.unit.isPublished ||
      !assignment.lesson.unit.course.isPublished ||
      assignment.lesson.unit.course.subject !== "اللغة العربية"
    ) {
      return NextResponse.json(
        {
          success: false,
    ...
    const answerData = assignment.questions.map((question) => {
    ...
    return NextResponse.json(
      {
        success: true,
        message: "تم إرسال الواجب وحساب النتيجة بنجاح.",
    ...
    return NextResponse.json(
      {
        success: false,
        message: "حدث خطأ أثناء إرسال الواجب.",
      },
      { status: 500 }
    );
  }
